...
alt.onServer('Doors:Create', (range)=>{
    ...
    doors.forEach((e)=>{
        if (canUse) {
            door = native.getClosestObjectOfType(alt.Player.local.pos.x, alt.Player.local.pos.y, alt.Player.local.pos.z, range, alt.hash(e), false, false, false);
            doorCode = alt.hash(e);
            if (door != 0) {
                canUse = false;
    ...
    obj.forEach((e)=>{
        if (e) {
            if (distance(alt.Player.local.pos, e.pos) < 20) {
                native.setLockedUnstreamedInDoorOfType(e.hash, e.pos.x, e.pos.y, e.pos.z, e.state, 0.0, 0.0, 0.0);
            }
        }
    });
}, 1000);
