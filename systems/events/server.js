...
import { setPlayerOutVehicle, setPlayerToVehicle } from '../../utils/functions';
...
function getSex() {
    if (alt.Player.local.model == alt.hash("mp_f_freemode_01")) return 0;
    else return 1;
}
alt.onServer('SetClothes', (a, b, c)=>{
    ...
    //     }
    // }
    native.setPedComponentVariation(alt.Player.local.scriptID, component, drawable, texture, 2);
});
...
function ShowIconNotification(message, title, subtitle, icon) {
    return _ShowIconNotification.apply(this, arguments);
}
function _ShowIconNotification() {
    _ShowIconNotification = _asyncToGenerator(function*(message, title, subtitle, icon, color = undefined, blink = false) {
        if (icon != null) {
            let pedHeadshot = native.registerPedheadshot(icon.scriptID);
            icon = yield loadPlayerHead(pedHeadshot);
        }
...
alt.onServer('teleportToWaypoint', ()=>{
    if (!native.isWaypointActive()) return alt.emit('chat:message', null, "{DF1111}[HATA] Öncelikle bir waypoint işaretlemelisiniz!");
    let player = alt.Player.local;
...
alt.onServer("Faction:CheckClothes", (name)=>{
    ...
    for(let a = 0; a <= 3; a++){
        let id1 = native.getPedPropIndex(ped, a);
        let texture1 = native.getPedPropTextureIndex(ped, a);
        let add1 = {
            prop: a,
            ID: id1,
            texture: texture1
...
alt.onServer("Player:CheckClothes", (name)=>{
    ...
    for(let a = 0; a <= 3; a++){
        if (a == 3) {
            ...
            let id2 = native.getPedPropIndex(ped, a);
            let texture2 = native.getPedPropTextureIndex(ped, a);
            let add2 = {
                prop: a,
                ID: id2,
                texture: texture2
...
alt.onServer('News:StartCamera', (id)=>{
    ...
    _camInterval = alt.everyTick(()=>{
        if (native.isEntityAttachedToEntity(player.scriptID, target.scriptID)) {
            if (!currentCam) {
                currentCam = native.createCamWithParams('DEFAULT_SCRIPTED_CAMERA', target.pos.x, target.pos.y, target.pos.z + 1, target.rot.x, target.rot.y, target.rot.z, 60, true, 0);
            }
...
alt.onServer('Show:VehicleModify', (id)=>{
    ...
    let spoiler = native.getVehicleMod(veh, 0);
    let frontBumber = native.getVehicleMod(veh, 1);
    let rearBumber = native.getVehicleMod(veh, 2);
    let sideSkirt = native.getVehicleMod(veh, 3);
    let exhaust = native.getVehicleMod(veh, 4);
    let frame = native.getVehicleMod(veh, 5);
    let grille = native.getVehicleMod(veh, 6);
    let bonnet = native.getVehicleMod(veh, 7);
    let wingleft = native.getVehicleMod(veh, 8);
    let wingright = native.getVehicleMod(veh, 9);
    let roof = native.getVehicleMod(veh, 10);
    let engine = native.getVehicleMod(veh, 11);
    let brakes = native.getVehicleMod(veh, 12);
    let horns = native.getVehicleMod(veh, 14);
    let suspension = native.getVehicleMod(veh, 15);
    let armor = native.getVehicleMod(veh, 16);
    let turbo = native.getVehicleMod(veh, 18);
    let xenon = native.getVehicleMod(veh, 22);
    let livery = native.getVehicleMod(veh, 48);
    let transmission = native.getVehicleMod(veh, 13);
    let text = "{DF1111}<center>Araca Ait Modifiyeler</center><br>Spoiler: " + getModifyText(spoiler) + " | Ön Tampon: " + getModifyText(frontBumber) + " | Arka Tampon " + getModifyText(rearBumber) + " | Etek: " + getModifyText(sideSkirt) + " | Egzos: " + getModifyText(exhaust) + " | Frame: " + getModifyText(frame) + " | Izgara: " + getModifyText(grille) + " | Kaput: " + getModifyText(bonnet) + " | Etek Yan: " + getModifyText(wingleft) + " | Etek Yan 2: " + getModifyText(wingright) + " | Çatı: " + getModifyText(roof) + " | Motor: " + getModifyText(engine) + " | Frenler: " + getModifyText(brakes) + " | Korna: " + getModifyText(horns) + " | Süspansiyon: " + getModifyText(suspension) + " | Zırh: " + getModifyText(armor) + " | Turbo: " + getModifyText(turbo) + " | Xenon: " + getModifyText(xenon) + " | Livery: " + getModifyText(livery) + " | Şanzıman: " + getModifyText(transmission);
    alt.emit('chat:message', null, text);
});
function getModifyText(modLevel) {
    if (modLevel < 0) return "Yok";
    else return "Var(Seviye: " + (modLevel + 1) + ")";
}
...
alt.onServer('Spec:OFF', (pos)=>{
    ...
    native.setPedCanBeTargetted(alt.Player.local.scriptID, true);
    native.setEntityCanBeDamaged(alt.Player.local.scriptID, true);
    native.setEntityCoords(alt.Player.local.scriptID, pos.x, pos.y, pos.z, false, false, false, true);
...
alt.onServer('Controls:Disable', ()=>{
    if (_ControlDisable) alt.clearInterval(_ControlDisable);
...
function leftBottomNotifyShow(message, flashing = false, textColor = -1, bgColor = -1, flashColor = [
    0,
    0,
    0,
    50
    ...
    if (bgColor > -1) native.thefeedSetBackgroundColorForNextPost(bgColor);
    if (flashing) {
        native.thefeedSetRgbaParameterForNextMessage(flashColor[0], flashColor[1], flashColor[2], flashColor[3]);
    }
    native.addTextComponentSubstringPlayerName(message);
    native.endTextCommandThefeedPostTicker(flashing, true);
}
alt.onServer('Noti:ShowPicture', leftBottomNotifyShowWithPicture);
function leftBottomNotifyShowWithPicture(title, sender, message, notifPic, iconType = 0, flashing = false, textColor = -1, bgColor = -1, flashColor = [
    0,
    0,
    0,
    50
...
alt.onServer('FacialAnimation', (type)=>{
    switch(type){
        case 1:
            var animation = "mood_angry_1";
            break;
        case 2:
            var animation = "mood_drunk_1";
            break;
        case 3:
            var animation = "pose_injured_1";
            break;
        case 4:
            var animation = "electrocuted_1";
            break;
        case 5:
            var animation = "effort_1";
            break;
        case 6:
            var animation = "mood_drivefast_1";
            break;
        case 7:
            var animation = "pose_angry_1";
            break;
        case 8:
            var animation = "mood_happy_1";
            break;
        case 9:
            var animation = "mood_injured_1";
            break;
        case 10:
            var animation = "mood_dancing_low_1";
            break;
        case 11:
            var animation = "smoking_hold_1";
            break;
        case 12:
            var animation = "pose_normal_1";
            break;
        case 13:
            var animation = "pose_aiming_1";
            break;
        case 14:
            var animation = "shocked_1";
            break;
        case 15:
            var animation = "shocked_2";
            break;
        case 16:
            var animation = "mood_sleeping_1";
            break;
        case 17:
            var animation = "dead_1";
            break;
        case 18:
            var animation = "dead_2";
            break;
        case 19:
            var animation = "mood_smug_1";
            break;
        case 20:
            var animation = "mood_aiming_1";
            break;
        case 21:
            var animation = "mood_stressed_1";
            break;
        case 22:
            var animation = "mood_sulk_1";
            break;
        case 23:
            var animation = "effort_2";
            break;
        case 24:
            var animation = "effort_3";
            break;
        default:
            var animation = "mood_angry_1";
            break;
