...
alt.onServer('GasStation:Start', (veh) => {
    if(!inGas){
        ...
        if(canBuy == null || canBuy == 0){
            ...
            native.attachEntityToEntity(gaspump, player.scriptID, native.getPedBoneIndex(player.scriptID, 58866), 0.075, -0.029, 0, 37.5, 0, 216.1, true, true, false, true, 1 ,true);
            let pumpipumpi = native.getWorldPositionOfEntityBone(player.scriptID, native.getPedBoneIndex(player.scriptID, 58866));
            rope = native.addRope(player.pos.x, player.pos.y, player.pos.z, 
                0.0,0.0,0.0,0.5,5,10.0,0.5,0.0,false,false,false,5.0,false,-1)[0];        
