        ...
        interval = alt.setInterval(()=>{
            if (IsHeliHighEnough(heli)) {
                ...
                if (native.isControlJustPressed(0, toggle_rappel)) {
                    if (native.getPedInVehicleSeat(heli, -1, false) == lPed || native.getPedInVehicleSeat(heli, 1, false) == lPed) {
                        native.playSoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
            ...
            if (helicam) {
                ...
                if (!cam) cam = native.createCam("DEFAULT_SCRIPTED_FLY_CAMERA", true);
                native.attachCamToEntity(cam, heli, 0.0, 0.0, -1.5, true);
                native.setCamRot(cam, 0.0, 0.0, native.getEntityHeading(heli), 0);
...
function isPlayerInPolmav() {
    if (alt.Player.local.vehicle.model == polmav_hash) return true;
    else return false;
}
...
function RenderVehicleInfo(vehicle) {
    ...
    let vehname = native.getFilenameForAudioConversation(native.getDisplayNameFromVehicleModel(model));
...
function getEntityFromRaycast(flag, ignorePlayer) {
    ...
    for(let i = 1; i < 5; i++){
        ...
        let ray;
        if (ignorePlayer) {
            ray = native.startExpensiveSynchronousShapeTestLosProbe(pos.x, pos.y, pos.z, posFront.x, posFront.y, posFront.z, flag, alt.Player.local.vehicle.scriptID, 0);
        } else {
            ray = native.startExpensiveSynchronousShapeTestLosProbe(pos.x, pos.y, pos.z, posFront.x, posFront.y, posFront.z, flag, undefined, 0);
        }
        // Get the Result
        // eslint-disable-next-line no-unused-vars
        let [_, _hit, _endCoords, _surfaceNormal, _entity] = native.getShapeTestResult(ray);
