...
alt.everyTick(()=>{
if(native.isPedShooting(alt.Player.local.scriptID) && IsInAnimation) EmoteCancel();
...
if(PtfxPrompt){
    ...
    if(native.IsControlPressed(0, 47)){
        PtfxStart();
        alt.setTimeout(()=>{
            PtfxStop();
        }, 500);
    ...
    if(Config.Config.MenuKeybindEnabled && native.IsControlPressed(0, Config.Config.MenuKeybind)) OpenEmoteMenu();
    if(Config.Config.EnableXtoCancel && native.IsControlPressed(0, 73)) EmoteCancel();
}
});
...
function EmoteCancel(){
    if(ChosenDict == "MaleScenario" && IsInAnimation){
        native.clearPedTasksImmediately(alt.Player.local.scriptID);
        IsInAnimation = false;
    }
    else if(ChosenDict == "Scenario" && IsInAnimation){
        IsInAnimation = false;
    }
...
function PtfxStart(){
  if (PtfxNoProp){
    PtfxAt = alt.Player.local.scriptID;
  }
  else{ PtfxAt = prop }
  native.UseParcileFxAssetNextCall(PtfxAsset);
  Ptfx = native.startNetworkedParticleFxLoopedOnEntityBone(PtfxName, PtfxAt, Ptfx1, Ptfx2, Ptfx3, Ptfx4, Ptfx5, Ptfx6, native.getEntityBoneIndexByName(PtfxName, "VFX"), 1065353216, 0, 0, 0, 1065353216, 1065353216, 1065353216, 0);
  native.setParticleFxLoopedColour(Ptfx, 1.0, 1.0, 1.0);
  table.insert(PlayerPaticles, Ptfx);
}

function PtfxStop(){
  pairs(PlayerParticles).forEach(element => {
    native.stopParticleFxLooped(element.b, false);
    table.remove(PlayerParticles, element.a);
  });
}
/*
