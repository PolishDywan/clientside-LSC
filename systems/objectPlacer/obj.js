...
function showAlphaObject(object) {
    requestModelPromise(game.getHashKey(object)).then((succ)=>{
        if (succ) {
            newObjectToPlace = game.createObjectNoOffset(game.getHashKey(object), player.pos.x, player.pos.y, player.pos.z, false, false, true);
            ...
            tickHandler = alt.everyTick(()=>{
                ...
                if (game.isControlPressed(2, 27)) {
                    if (alphaMode) {
                        ...
                        game.setEntityAlpha(newObjectToPlace, game.getEntityAlpha(newObjectToPlace) + 1, true);
                    } else {
                ...
                if (game.isControlPressed(2, 108)) {
                    ...
                    game.setEntityRotation(newObjectToPlace, objectRot.x, objectRot.y, objectRot.z + 0.5, 2, false);
                }
                //turn right
                if (game.isControlPressed(2, 109)) {
                    let objectRot1 = game.getEntityRotation(newObjectToPlace, 2);
                    game.setEntityRotation(newObjectToPlace, objectRot1.x, objectRot1.y, objectRot1.z - 0.5, 2, false);
                }
                ...
                if (newObjectToPlace !== null) {
                    game.setEntityCoords(newObjectToPlace, posFront.x, posFront.y, player.pos.z + placeObjeGround, false, false, false, false);
                }
            });
        }
    });
