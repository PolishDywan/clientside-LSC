...
//
alt.onServer("PD:Teckle", (data1, data2) =>{
let player1 = alt.Player.getByID(data1);
let player2 = alt.Player.getByID(data2);
    game.attachEntityToEntity(player1, player2, 11816, 0.25, 0.5, 0.0, 0.5, 0.5, 180.0, false, false, false, false, 2, false);
        setTimeout(()=>{
            game.detachEntity( player1, true, false);
        }, 3000);
});
// ----------- Interface Functions ----------------------
alt.onServer("Server:Shield:giveShield", () => {
    let riotshield = alt.Player.local.getStreamSyncedMeta("shield") == 1;
    createAndAttachShield(alt.Player.local, riotshield);
    shield = true;
    console.log("test11");
});

alt.onServer("Server:Shield:removeShield", () => {
