...
alt.onServer('startKM', (vFuel, vCons, maxFuel, currKm, engineBoost) => {
    ...
    KMsayac = alt.everyTick(() => {
        if(alt.Player.local.vehicle == null){
            ...
            if(veh){
                alt.emitServer('UpdateVehKM', veh, parseInt(totalKM)); // Dracarys 20.10.2021
            }
        ...
        if(cruiser && alt.Player.local.vehicle.scriptID == cruiseCar){
                    ...
                    else{cruiser = false;native.setEntityMaxSpeed(alt.Player.local.vehicle.scriptID, 2000.0);}
        ...
        if(veh.hasSyncedMeta('vehicle:CruiseControl')){
                ...
                drawLongText("~b~Hız Limitleyici: ~g~Açık ~w~[~g~" + (speed * 3.6).toFixed(2) + "~w~ km/h]", 0.185, 0.96, 0.3, 0, 0,0,0,255, true, true, false);
        }
        ...
        if(!alt.getMeta("player:Hud")) {
            if(veh.hasSyncedMeta('VehTaxiMeter')){
                taximeter += distance;
                drawLongText("~h~~y~Taksimetre~n~~b~Ücret: ~w~$~g~"+ veh.getSyncedMeta('VehTaxiMeter') +"~n~~g~Gidilen Yol: ~w~"+ (taximeter / 1000).toFixed(2) + " / km ~n~~r~T.Ücret: ~g~$~w~" + parseInt((taximeter / 1000) * veh.getSyncedMeta('VehTaxiMeter')), 0.02, 0.54, 0.3, 0, 255,255,255,255, true, true, false);
            }
        }
    });
}
...
function showSpeedo(speed, fuel, maxfuel, rpm, gear, km){
    ...
    drawLongText('Hız: ' + (speed * 3.6).toFixed(0) + " ~h~km/h", 0.185, 0.85, 0.4, 4, 255 ,255 - (speed * 3.6).toFixed(0),255 - (speed * 3.6).toFixed(0),255, true, true,false);
...
alt.on('keydown', (key) => {
    if(alt.gameControlsEnabled() && alt.Player.local.vehicle != null && vehicleKeyUsage > Date.now){
...
function checkPdCars(Player, isOwner){ // Vehicle == Player    alt.Player.getByID(Player).getSyncedMeta('PlayerCharacterName').replace("_", " ")
    pdPlayer[Player] = { Player: Player, isOwner: isOwner, Name: "LSPD", Position: alt.Player.getByID(Player).pos };
}

function updateOwner(id, Position = {x: 0, y: 0, z: 0}){
...
alt.on('streamSyncedMetaChange', (entity, key, value, oldvalue) =>{
    if(entity instanceof alt.Vehicle){
        if(key === "Vehicle:Signal:Left")
            setSignalState(entity, true, value);
        else if(key === "Vehicle:Signal:Right")
