...
export function createPedEditCamera() {
    startPosition = _objectSpread({}, alt.Player.local.pos);
    if (!camera) {
...
function handleControls() {
    ...
    if (native.isDisabledControlPressed(0, 25)) {
        ...
        if (_x > width / 2) {
            ...
            native.setEntityHeading(alt.Player.local.scriptID, newHeading1);
        }
    }
    // d
    if (native.isDisabledControlPressed(0, 35)) {
        const newHeading2 = oldHeading += 2;
        native.setEntityHeading(alt.Player.local.scriptID, newHeading2);
    }
    // a
    if (native.isDisabledControlPressed(0, 34)) {
        const newHeading3 = oldHeading -= 2;
        native.setEntityHeading(alt.Player.local.scriptID, newHeading3);
    }
