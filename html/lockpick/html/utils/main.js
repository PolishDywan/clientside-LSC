var minRot = -90, maxRot = 90, solveDeg = Math.random() * 180 - 90, solvePadding = 4, maxDistFromSolve = 45, pinRot = 0, cylRot = 0, lastMousePos = 0, mouseSmoothing = 2, keyRepeatRate = 25, cylRotSpeed = 3, pinDamage = 20, pinHealth = 100, pinDamageInterval = 150, numPins = 2, userPushingCyl = false, gameOver = false, gamePaused = false, pin, cyl, driver, cylRotationInterval, pinLastDamaged;
$(function() {
    //pop vars
    pin = $('#pin');
    cyl = $('#cylinder');
...
function outOfPins() {
    ...
    //$('#modal').fadeIn();
    if ('alt' in window) alt.emit('no');
}
function unlock() {
    gameOver = true;
    //$('#win').css('display','inline-block');
    //$('#modal').fadeIn();
    if ('alt' in window) alt.emit('succes');
}
//UTIL
Util = {};
