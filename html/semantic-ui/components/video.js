(function($, window1, document1, undefined) {
    ...
    $.fn.video = function(parameters) {
        ...
        $allModules.each(function() {
            var settings = $.isPlainObject(parameters) ? $.extend(true, {}, $.fn.video.settings, parameters) : $.extend({}, $.fn.video.settings), selector = settings.selector, className = settings.className, error = settings.error, metadata = settings.metadata, namespace = settings.namespace, templates = settings.templates, eventNamespace = '.' + namespace, moduleNamespace = 'module-' + namespace, $window = $(window1), $module = $(this), $placeholder = $module.find(selector.placeholder), $playButton = $module.find(selector.playButton), $embed = $module.find(selector.embed), element = this, instance = $module.data(moduleNamespace), module;
            module = {
                initialize: function() {
                ...
                create: function() {
                    var image = $module.data(metadata.image), html = templates.video(image);
                    $module.html(html);
                    module.refresh();
                    if (!image) {
                        module.play();
                ...
                play: function() {
                    ...
                    var source = $module.data(metadata.source) || false, url = $module.data(metadata.url) || false, id = $module.data(metadata.id) || false;
                ...
                generate: {
                    ...
                    url: function(source) {
                        var api = settings.api ? 1 : 0, autoplay = settings.autoplay === 'auto' ? $module.data('image') !== undefined : settings.autoplay, hd = settings.hd ? 1 : 0, showUI = settings.showUI ? 1 : 0, // opposite used for some params
                        hideUI = !settings.showUI ? 1 : 0, url = '';
                        if (source == 'vimeo') {
