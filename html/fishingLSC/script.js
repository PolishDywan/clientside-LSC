...
(function() {
    ...
    class Indicator {
        ...
        detectCollision() {
            if (fish.y < this.y && fish.y > this.y - this.height || fish.y - fish.height < this.y && fish.y - fish.height > this.y - this.height) {
                progressBar.fill();
    ...
    class Fish {
        ...
        updateFishPosition() {
            if (!this.randomPosition || this.randomCountdown < 0) {
                ...
                this.speed = Math.abs((Math.random() + 1) * 0.4); // BURAYA ONUR HIZ VERECEK.
            }
    ...
    class ProgressBar {
        ...
        detectGameEnd() {
            if (this.progress >= 100) {
                ...
                gameOver = true;
                if ('alt' in window) alt.emit('Fishing:Succes');
            }
        }
        updateUi() {
    ...
    document.querySelector('#kapat').addEventListener("click", ()=>{
        if ('alt' in window) alt.emit('Fishing:PageClose');
    });
    ...
    class Seaweed {
        draw() {
            ...
            for(let i = this.segments; i >= 0; i--){
                if (i === this.segments) {
                    context.moveTo(Math.sin(this.sin + i) * i / 2.5 + this.xoff, canvas.height + -i * this.segmentSpread);
                } else {
                    context.lineTo(Math.sin(this.sin + i) * i / 2.5 + this.xoff, canvas.height + -i * this.segmentSpread);
                }
...
(function() {
    ...
    class Line {
        draw() {
            ...
            context.bezierCurveTo(canvas.width, canvas.height / 2 + this.tension, canvas.width / 2, canvas.height + this.tension, 0, canvas.height);
            context.stroke();
        }
        animate() {
