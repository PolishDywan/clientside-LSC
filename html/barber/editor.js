...
const url = `http://resource/client/html/barber/html/index.html`;
let view;
let oldData = {};
let prevData = {};
let readyInterval;
...
alt.onServer('character:Barber', handleEdit);
...
function handleDone(newData) {
    alt.emitServer('login:SetupComp', JSON.stringify(newData));
    closeView();
    alt.emit('login:callUpdate');
}
function handleCancel() {
    alt.emitServer('login:SetupComp', JSON.stringify(oldData));
    closeView();
    alt.emit('login:callUpdate');
}
